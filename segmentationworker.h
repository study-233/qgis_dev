...
protected:
    void run() override {
        ...
        // 加载模块
        qDebug() << "Loading module 'sem_seg'...";
        PyObject *pModule = PyImport_ImportModule("sem_seg");
        if (!pModule) {
            qDebug() << "Failed to load module 'sem_seg'.";
            PyErr_Print();  // 打印错误信息
        ...
        // 获取函数
        qDebug() << "Getting function 'sem_seg'...";
        PyObject *pFunc = PyObject_GetAttrString(pModule, "sem_seg");
        if (!pFunc || !PyCallable_Check(pFunc)) {
            qDebug() << "Function 'sem_seg' not callable or not found.";
            PyErr_Print();  // 打印错误信息
            Py_XDECREF(pModule);
            emit processingFinished(false);
            return;
        ...
        // 调用Python函数
        qDebug() << "Calling Python function 'sem_seg'...";
